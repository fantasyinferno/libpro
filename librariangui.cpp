...
LibrarianGUI::LibrarianGUI(QWidget *parent, QSqlDatabase database) :
    QMainWindow(parent),
...
QSqlDatabase LibrarianGUI::getDatabase() {
    return db;
}
QString LibrarianGUI::getUser() {
    return user;
}

void LibrarianGUI::on_thanhTimKiem_returnPressed()
{
...
void LibrarianGUI::initializeDatabase(QSqlDatabase database = QSqlDatabase()) {
    db = database;
}

void LibrarianGUI::initializeTable()
{
    // Thiết lập model
...
void LibrarianGUI::initializeQuotes() {
...
void LibrarianGUI::initializeGUILogic(QSqlDatabase database) {
...
void LibrarianGUI::on_dangXuatButton_clicked()
{
    user = "";
    ui->username->setText("");
    ui->username->setEnabled(false);
...
void LibrarianGUI::on_dangNhapButton_clicked()
{
    emit formRequest(0);
}

void LibrarianGUI::on_dangKyButton_clicked()
{
    emit formRequest(1);
}
void LibrarianGUI::on_username_clicked() {
    emit informationRequest();
}

void LibrarianGUI::on_dangNhapThanhCong(int id, QString username) {
    user = username;
    user_id = id;
...
void LibrarianGUI::on_avatarChanged(const QPixmap* pixmap) {
    ui->avatarIcon->setPixmap(*pixmap);
}

void LibrarianGUI::on_chuyenVaiTroButton_clicked()
{
    emit chuyenVaiTro();
}
